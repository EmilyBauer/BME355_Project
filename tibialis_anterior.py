...
class FootDynamics:
    def __init__(self, forces, times):
    ...
    def get_angular_acceleration_foot(self):        # derivative of w_f
        return ((self.get_TA_torque() + self.get_gravity_torque() + self.mass_foot*self.foot_center*(self.get_ankle_x_acceleration()*math.sin(self.get_angle_foot(time)) - self.get_ankle_y_acceleration()*math.cos(self.get_angle_foot(time))))/(self.get_moment_of_inertia + self.mass_foot * (self.foot_center)**2))

    def get_angle_foot(self,t):   # alphaF is the angular motion of the foot. alphaF = 0 when foot is horizontal
        #get the integral of angular acceleration
        #get the integral of that integral
        foot_angle_list = []
        for i in t:
            foot_angle_list.append(self.get_angular_acceleration_foot(), t, t+0.001)
        return foot_angle_list
    ...
    def get_ankle_x_acceleration(t): # ax,A  is the acceleration of the ankle in the x-direction
        return 29692.8*pow(t,3) - 14349.9*pow(t,2) + 1661.62*t + 19.986
    
    def get_ankle_y_acceleration(t): # aY,A is the acceleration of the ankle in the y-direction
        return 8810*pow(t,3) - 4239.6*pow(t,2) + 447.8*t + 1.0081
...
if __name__ == '__main__':
    ...
    def activation():
        ...
        activation_sol = solve_ivp(f, [0, 0.3], [0.3], max_step=.01, rtol=1e-5,
                                   atol=1e-8)  # I don't know what the initial value should be
    ...
    length_sol = solve_ivp(f, [0, 0.3], [1], max_step=.01, rtol=1e-5, atol=1e-8)  # initial force?

    TA_force = muscle.get_force(total_length, length_sol.y.T)

    print(TA_force)

    foot = FootDynamics(TA_force, activation_times)
    foot_angles = foot.get_angle_foot(activation_times)
