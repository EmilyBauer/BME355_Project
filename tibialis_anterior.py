...
class FootDynamics:
    def __init__(self, forces_equation):
        ...
        self.force_equation = forces_equation
        self.moment_of_inertia = (self.mass_foot*0.26)/12  # assuming the foot to be a thin rod, I = Ml^2/12
                                                           # using the journal of eng science and tech paper:
                                                           # length of foot is 0.26 m
        

    def get_angle_foot(self):
        def f(t, Y):
            return(Y[1], (self.get_TA_torque(t) + -1*self.foot_center * math.cos(Y[0]) * self.mass_foot * 9.81 + self.mass_foot*self.foot_center*(self.get_ankle_x_acceleration(t)*math.sin(Y[0]) - self.get_ankle_y_acceleration(t)*math.cos(Y[0])))/(self.moment_of_inertia + self.mass_foot*(self.foot_center)**2))

        sol = solve_ivp(f, [0, 0.3], [-1.5708, -1], max_step=.01, rtol=1e-5, atol=1e-8)
        return (sol.y[0], sol.t)

    def get_ankle_x_acceleration(self, time): # ax,A  is the acceleration of the ankle in the x-direction
        return 29692.8*pow(time,3) - 14349.9*pow(time,2) + 1661.62*time + 19.986

    def get_ankle_y_acceleration(self, time): # aY,A is the acceleration of the ankle in the y-direction
        return 8810*pow(time,3) - 4239.6*pow(time,2) + 447.8*time + 1.0081

    def get_TA_torque(self, time): #TTA is the torque generated by the tibialis anterior
        return self.force_equation.eval(time) * self.lever_arm
...
def get_velocity(a, lm, lt):
    ...
    def funk(vm):
        return (a * force_length_muscle(lm) * force_velocity_muscle(vm) + force_length_parallel(
            lm) + beta * vm) - force_length_tendon(lt)
...
if __name__ == '__main__':
    resting_muscle_length = 0.3  # fill out with TA values
    resting_tendon_length = 0.1  # fill out with TA values
    max_isometric_force = 100  # fill out with TA values
    muscle = HillTypeMuscle(max_isometric_force, resting_muscle_length, resting_tendon_length)
    time_deactivation = 0.04
    time_activation = 0.01
    ...
    def activation():
        ...
        activation_sol = solve_ivp(f, [0, 0.3], [0.3], max_step=.0001, rtol=1e-5, atol=1e-8)  # I don't know what the initial value should be
        return activation_sol.t, activation_sol.y


    def get_activation_regression(times, activations):
        centres = np.arange(0, 1, 0.1)
        width = 0.15
        regression_model = Regression(times, activations, centres, width, 0.1, sigmoids=True)
        return regression_model


    activation_values = activation()

    activation_times = activation_values[0]
    activations = activation_values[1]
    activations = activations[0]

    activation_regression_model = get_activation_regression(activation_times, activations)


    def f(t, lm):
        activation = activation_regression_model.eval(t)
        return get_velocity(activation, lm, muscle.norm_tendon_length(total_length, lm))


    length_sol = solve_ivp(f, [0, 0.3], [1], max_step=.01, rtol=1e-5, atol=1e-8)

    TA_force = muscle.get_force(total_length, length_sol.y.T)

    def get_FTA_regression(times, TA_force):
        centres = np.arange(0, 1, 0.1)
        width = 0.15
        regression_model = Regression(times, TA_force, centres, width, 0.1, sigmoids=True)
        return regression_model

    #print(TA_force)

    TA_force_regression = get_FTA_regression(length_sol.t, TA_force)

    foot = FootDynamics(TA_force_regression)
    (foot_angles, foot_times) = foot.get_angle_foot()
    print(foot_angles)
    ...
    plt.figure()
    plt.plot(length_sol.t, TA_force)
    plt.xlabel('Time (s)')
    plt.ylabel('Force')
    plt.tight_layout()
    plt.show()
    ...
    plt.figure()
    plt.plot(foot_times, foot_angles)
    plt.xlabel('Time (s)')
