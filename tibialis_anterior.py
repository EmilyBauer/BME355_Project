...
class FootDynamics:
    ...
    def get_angle_foot(self):
        def f(t, Y):
            return (Y[1], (self.get_TA_torque(t) + -1 * self.foot_center * math.cos(
                Y[0]) * self.mass_foot * 9.81 + self.mass_foot * self.foot_center * (
                                       self.get_ankle_x_acceleration(t) * math.sin(
                                   Y[0]) - self.get_ankle_y_acceleration(t) * math.cos(Y[0]))) / (
                                self.moment_of_inertia + self.mass_foot * (self.foot_center) ** 2))
...
class HillTypeMuscle:
    ...
    def norm_tendon_length(self, muscle_tendon_length, normalized_muscle_length):
        ...
        """
        return (
                           muscle_tendon_length - self.resting_length_muscle * normalized_muscle_length) / self.resting_length_tendon
...
if __name__ == '__main__':
    ...
    def f(t, Y):
        ...
        # dY[1] = velocity
        # Y[1] = length
        # dY[2] = angular velocity = Y[3]
        # Y[2] = angle
        # dY[3] = angular acceleration
        # Y[3] = angular velocity

        a_min = 0.0001
        a_hat = (Y[0] - a_min) / (1 - a_min)
        if (muscle_excitation_1(t) > Y[0]):
            tau = time_activation * (0.5 + 1.5 * a_hat)
        else:
            tau = time_deactivation / (0.5 + 1.5 * a_hat)
        dY[0] = (muscle_excitation_1(t) - a_hat) / tau  # activation_dot
        dY[1] = get_velocity(Y[0], Y[1], muscle.norm_tendon_length(total_length, Y[1]))
        dY[2] = Y[3]
        dY[3] = (muscle.get_force(total_length, Y[1]) * foot.lever_arm + -1 * foot.foot_center * math.cos(Y[2]) * foot.mass_foot * 9.81 + foot.mass_foot * foot.foot_center * (foot.get_ankle_x_acceleration(t) * math.sin(Y[2]) - foot.get_ankle_y_acceleration(t) * math.cos(Y[2]))) / (foot.moment_of_inertia + foot.mass_foot * (foot.foot_center) ** 2)
        return dY


    sol = solve_ivp(f, [0, 0.3], [0.3, 1, -0.785, -5.65], max_step=0.00001, rtol=1e-5, atol=1e-8)
    activations = sol.y[0]
    force = muscle.get_force(total_length, sol.y[1].T)
    foot_angles = sol.y[2]
    foot_angular_velocities = sol.y[3]
    time_array = sol.t

    plt.figure()
    plt.subplot(4,1,1)
    plt.plot(time_array, activations)
    plt.ylabel('Activation')
    plt.subplot(4,1,2)
    plt.plot(time_array, force)
    plt.ylabel('Force')
    plt.subplot(4,1,3)
    plt.plot(time_array, foot_angles)
    plt.ylabel('Angle')
    plt.subplot(4,1,4)
    plt.plot(time_array, foot_angular_velocities)
    plt.xlabel('Time (s)')
