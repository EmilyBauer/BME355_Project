...
lMT # is the length of the muscle-tendon complex for the tibialis anterior. lMT = lMT,0 - dankle*theta
lMT,0 # is the rest length.  lMT,0 = 0.381
theta # is the current relative angle at the foot. theta :=  alphaS - alphaF. theta = 0 when there is a right angle at the ankle
lT # is the tendon length constant. lT = 0.317
lCE # is the length of the contractile element. lCE = lMT - lT  
VCE # is the contraction speed of the muscle. VCE =  - dankle*theta (*** the phi has a lil dot on him but idk how to draw it)
...
class SwingPhase:
    def __init__(self, muscle_excitation, theta, soleus_length):
        ...
        self.tendon_length = 0.317
        self.time_activation = 
        self.gmax = 10          # idk what this is for
        slef.LCE_opt = 0.82     # also don't know
    ...
    def get_TA_torque(): #TTA is the torque generated by the tibialis anterior
        return get_TA_force() * self.lever_arm
    ...
    def get_activation_level(): # fad is the activation level. 0 ≤ fad ≤ 1
        return 
    def force_length_factor(): # I think we might have to use webplotdigitizer for this
        return
    def force_velocity_factor(): # I think we might have to use webplotdigitizer for this
        return
    def get_relative_angle_of_foot():
        return
    def get_MT_length():
        return self.resting_MT_length - self.lever_arm*get_relative_angle_of_foot()
    def get_CE_length():
        return get_MT_length() - self.tendon_length


fad # is the activation level. 0 ≤ fad ≤ 1
muscle_excitation # is the muscle excitation. 0 ≤ muscle_excitation  ≤ 1        (epsilon??)
Tact # is the time constant for activation. Tact = 0.04s
Tdeact # is the time constant for deactivation. Tdeact = 0.04s     
lMT # is the length of the muscle-tendon complex for the tibialis anterior. lMT = lMT,0 - dankle*theta
lMT,0 # is the rest length.  lMT,0 = 0.381
